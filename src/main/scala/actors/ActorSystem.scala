...
trait ActorSystem {
  ...
  trait Actor {
    ...
    def !(message: Message): Unit =
      effects.set(
        effects.get.copy(messages = effects.get.messages :+ (message -> this)))
    ...
    final protected[actors] def process(behaviour: Behaviour,
                                        message: Message): Effects =
      effectsOf(behaviour) { () =>
        behaviour(message)
      }

    private[this] def effectsOf(behaviour: Behaviour)(
        action: () => Unit): Effects = {
  ...
  protected case class Effects(of: Actor,
                               behaviour: Behaviour,
    ...
    def amend(that: Effects): Effects = {
      ...
      copy(messages = this.messages ::: that.messages,
           actors = this.actors ::: that.actors)
    }
  }
