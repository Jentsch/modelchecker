...
The EcSpec provide the `everyPossiblePath` method, which allows to test every possible execution path.
...
${"Traverser" ~/ ecspec.`Traverser Specification`} - implements the traversation of all possible reachable states
