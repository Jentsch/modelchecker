...
final class Graph[E] private[Graph] (private val wrapped: XGraph[E, DiEdge]) {
  ...
  def withAncestors(nodes: Set[E], filter: E => Boolean = x => true): Set[E] = {
    ...
    def withAncestorsRec(unvisited: Set[NodeT],
                         nodes: Set[NodeT] = Set.empty[NodeT]): Set[NodeT] = {
...
object Graph {
  ...
  def explore[E: ClassTag](init: Traversable[E])(
      successors: E => Traversable[E]): Graph[E] = {

    @tailrec
    def depthFirstSearch(unvisited: List[E],
                         visited: Set[E],
                         graph: XGraph[E, DiEdge]): Graph[E] =
      unvisited match {
        case Nil =>
          new Graph(graph)
